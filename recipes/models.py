...
class Recipe(models.Model):
    ...
    # install pip install django-richtextfield
    instructions = RichTextField(max_length=10000, null=False, blank=False)
    ingredients = RichTextField(max_length=10000, null=False, blank=False)
    # pip install django_resized to resize the image
    #    pip install pillow package for images in django
    ...
    cuisine_types = models.CharField(
        max_length=100, choices=CUISINE_TYPES, default="african"
    )
    calories = models.IntegerField()
    posted_date = models.DateTimeField(auto_now=True)

    # allow us to order from the db
    class Meta:
